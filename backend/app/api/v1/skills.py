...
from app.services.skill_cache_service import skill_cache_service
...
async def list_skills(current_user: CurrentUser) -> BaseResponse[list[SkillSummaryVO]]:
    """获取已通过校验的 Skills 列表。"""
    return success(await skill_cache_service.list_summaries())
...
async def refresh_skills(admin: AdminUser) -> BaseResponse[SkillRefreshVO]:
    ...
    await skill_cache_service.publish_current_catalog()
...
async def get_skill_detail(name: str, current_user: CurrentUser) -> BaseResponse[SkillDetailVO]:
    ...
    try:
        return success(await skill_cache_service.detail(name))
