...
图示：比较RNN中计算梯度的策略，3行自上而下分别为：随机截断、常规截断、完整计算
...
图示：上图表示具有三个时间步的循环神经网络模型依赖关系的计算图。未着色的方框表示变量，着色的方框表示参数，圆表示运算符

正如刚才所说， 相关章节或公式中的模型参数是
...
图示：分割文本时，不同的偏移量会导致不同的子序列

因此，我们应该从 相关章节或公式中选择哪一个呢？
事实上，他们都一样的好。
然而，如果我们只选择一个偏移量，
...
事实上，我们还没有讨论多层循环神经网络的意义（这将在 相关章节或公式中介绍）。
...
它与我们训练 相关章节或公式模型的方式有三个不同之处。

1. 序列数据的不同采样方法（随机采样和顺序分区）将导致隐状态初始化的差异。
1. 我们在更新模型参数之前裁剪梯度。
   这样的操作的目的是，即使训练过程中某个点上发生了梯度爆炸，也能保证模型不会发散。
1. 我们用困惑度来评价模型。如 相关章节或公式所述，
...
因此 相关章节或公式的计算是*循环的*（recurrent）。
于是基于循环计算的隐状态神经网络被命名为
*循环神经网络*（recurrent neural network）。
在循环神经网络中执行 相关章节或公式计算的层
称为*循环层*（recurrent layer）。

有许多不同的方法可以构建循环神经网络，
由 相关章节或公式定义的隐状态的循环神经网络是非常常见的一种。
...
以及$\mathbf{b}_h$的偏置，所有这些参数都来自 相关章节或公式。
...
图示：基于循环神经网络的字符级语言模型：输入序列和标签序列分别为“machin”和“achine”

在训练过程中，我们对每个时间步的输出层的输出进行softmax操作，
然后利用交叉熵损失计算模型输出和标签之间的误差。
由于隐藏层中隐状态的循环计算，
 相关章节或公式中的第$3$个时间步的输出$\mathbf{O}_3$
由文本序列“m”“a”和“c”确定。
...
为了简单起见，我们以 相关章节或公式所示的股票价格（富时100指数）为例。
...
现在，准备[**训练模型**]了。实现下面的训练代码的方式与前面几节（如 相关章节或公式）中的循环训练基本相同。因此，我们不会深入探讨太多细节。
