...
使用`data_iter`的方式与我们在 相关章节或公式中使用`data_iter`函数的方式相同。为了验证是否正常工作，让我们读取并打印第一个小批量样本。
与 相关章节或公式不同，这里我们使用`iter`构造Python迭代器，并使用`next`从迭代器中获取第一项。
...
$$\mathrm{price} = w_{\mathrm{area}} \cdot \mathrm{area} + w_{\mathrm{age}} \cdot \mathrm{age} + b.$$
 ...
 相关章节或公式中的$w_{\mathrm{area}}$和$w_{\mathrm{age}}$
...
公式 相关章节或公式中的$\mathbf{w}$和$\mathbf{x}$都是向量。
...
在 相关章节或公式中，我们将线性回归模型描述为一个神经网络。
需要注意的是，该图只显示连接模式，即只显示每个输入如何连接到输出，隐去了权重和偏置的值。

图示：线性回归是一个单层神经网络。

在 相关章节或公式所示的神经网络中，输入为$x_1, \ldots, x_d$，
...
也就是说， 相关章节或公式中神经网络的*层数*为1。
我们可以将线性回归模型视为仅由单个人工神经元组成的神经网络，或称为单层神经网络。

对于线性回归，每个输入都与每个输出（在本例中只有一个输出）相连，
我们将这种变换（ 相关章节或公式中的输出层）
...
本节我们将使用刚刚在 相关章节或公式中引入的Fashion-MNIST数据集，
并设置数据迭代器的批量大小为256。
...
在 相关章节或公式中我们介绍了线性回归。
随后，在 相关章节或公式中我们从头实现线性回归。
然后，在 相关章节或公式中我们使用深度学习框架的高级API简洁实现线性回归。
