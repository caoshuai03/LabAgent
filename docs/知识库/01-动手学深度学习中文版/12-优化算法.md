...
Adadelta是AdaGrad的另一种变体（ 相关章节或公式），
...
以下是Adadelta的技术细节。鉴于参数du jour是$\rho$，我们获得了与 相关章节或公式类似的以下泄漏更新：

$$\begin{aligned}
    \mathbf{s}_t & = \rho \mathbf{s}_{t-1} + (1 - \rho) \mathbf{g}_t^2.
\end{aligned}$$

与 相关章节或公式的区别在于，我们使用重新缩放的梯度$\mathbf{g}_t'$执行更新，即
...
正如在 相关章节或公式中那样，我们可以根据其特征分解$\mathbf{Q} = \mathbf{U}^\top \boldsymbol{\Lambda} \mathbf{U}$重写这个问题，来得到一个简化得多的问题，使每个坐标都可以单独解出：
...
与 相关章节或公式一节中的实验相比，这里使用更大的学习率来训练模型。
...
* 在深度学习问题上，AdaGrad算法有时在降低学习率方面可能过于剧烈。我们将在 相关章节或公式一节讨论缓解这种情况的策略。
...
1. 尝试对适当的深度网络使用AdaGrad算法，例如，相关章节或公式中应用于Fashion-MNIST的深度网络。
...
* 在 相关章节或公式中，我们学习了：随机梯度下降在解决优化问题时比梯度下降更有效。
* 在 相关章节或公式中，我们学习了：在一个小批量中使用更大的观测值集，可以通过向量化提供额外效率。这是高效的多机、多GPU和整体并行处理的关键。
* 在 相关章节或公式中我们添加了一种机制，用于汇总过去梯度的历史以加速收敛。
* 在 相关章节或公式中，我们通过对每个坐标缩放来实现高效计算的预处理器。
* 在 相关章节或公式中，我们通过学习率的调整来分离每个坐标的缩放。
...
在 相关章节或公式中连接$a$和$b$的线段需要包含一部分既不在$\mathcal{X}$也不在$\mathcal{Y}$中。
...
事实上，我们一直在使用这个技巧。
比如权重衰减 相关章节或公式，在目标函数中加入$\frac{\lambda}{2} |\mathbf{w}|^2$，以确保$\mathbf{w}$不会增长太大。
...
同样，我们之前也遇到过，例如在 相关章节或公式中处理梯度截断时，我们通过
...
图示：Convex Projections.

投影的数学定义听起来可能有点抽象，为了解释得更清楚一些，请看 相关章节或公式。
图中有两个凸集，一个圆和一个菱形。
...
也就是说， 相关章节或公式中的$\mathcal{O}(\eta^2 f'^2(x))$可能变得显著了。
...
正如我们在 相关章节或公式中所看到的，选择“恰到好处”的学习率$\eta$是很棘手的。
...
毕竟，$f$的最小值满足$\nabla f = 0$。
遵循 相关章节或公式中的微积分规则，
通过取$\boldsymbol{\epsilon}$对 相关章节或公式的导数，
再忽略不重要的高阶项，我们便得到
...
* 首先，学习率的大小很重要。如果它太大，优化就会发散；如果它太小，训练就会需要过长时间，或者我们最终只能得到次优的结果。我们之前看到问题的条件数很重要（有关详细信息，请参见 相关章节或公式）。直观地说，这是最不敏感与最敏感方向的变化量的比率。
* 其次，衰减速率同样很重要。如果学习率持续过高，我们可能最终会在最小值附近弹跳，从而无法达到最优解。 相关章节或公式比较详细地讨论了这一点，在 相关章节或公式中我们则分析了性能保证。简而言之，我们希望速率衰减，但要比$\mathcal{O}(t^{-\frac{1}{2}})$慢，这样能成为解决凸问题的不错选择。
* 另一个同样重要的方面是初始化。这既涉及参数最初的设置方式（详情请参阅 相关章节或公式），又关系到它们最初的演变方式。这被戏称为*预热*（warmup），即我们最初开始向着解决方案迈进的速度有多快。一开始的大步可能没有好处，特别是因为最初的参数集是随机的。最初的更新方向可能也是毫无意义的。
...
此外，我们混合网络以提高性能。
由于大多数代码都是标准的，我们只介绍基础知识，而不做进一步的详细讨论。如果需要，请参阅 相关章节或公式进行复习。
...
到目前为止，我们在基于梯度的学习方法中遇到了两个极端情况：
 相关章节或公式中使用完整数据集来计算梯度并更新参数，
 相关章节或公式中一次处理一个训练样本来取得进展。
...
需要注意的是，在 相关章节或公式中，我们使用了一种在很大程度上取决于小批量中的方差的正则化。
...
在 相关章节或公式一节中，我们详述了如何执行随机梯度下降，即在只有嘈杂的梯度可用的情况下执行优化时会发生什么。
...
回想我们在 相关章节或公式中使用了$f(\mathbf{x}) = x_1^2 + 2 x_2^2$，即中度扭曲的椭球目标。
...
尽管优化提供了一种最大限度地减少深度学习损失函数的方法，但本质上，优化和深度学习的目标是根本不同的。前者主要关注的是最小化目标，后者则关注在给定有限数据量的情况下寻找合适的模型。在 相关章节或公式中，我们详细讨论了这两个目标之间的区别。例如，训练误差和泛化误差通常不同：由于优化算法的目标函数通常是基于训练数据集的损失函数，因此优化的目标是减少训练误差。但是，深度学习（或更广义地说，统计推断）的目标是减少泛化误差。为了实现后者，除了使用优化算法来减少训练误差之外，我们还需要注意过拟合。
...
为了说明上述不同的目标，引入两个概念*风险*和*经验风险*。如 相关章节或公式所述，经验风险是训练数据集的平均损失，而风险则是整个数据群的预期损失。下面我们定义了两个函数：风险函数`f`和经验风险函数`g`。假设我们只有有限的训练数据。因此，这里的`g`不如`f`平滑。
...
本章将关注优化算法在最小化目标函数方面的性能，而不是模型的泛化误差。在 相关章节或公式中，我们区分了优化问题中的解析解和数值解。在深度学习中，大多数目标函数都很复杂，没有解析解。相反，我们必须使用数值优化算法。本章中的优化算法都属于此类别。
...
可能遇到的最隐蔽问题是梯度消失。回想一下我们在 相关章节或公式中常用的激活函数及其衍生函数。例如，假设我们想最小化函数$f(x) = \tanh(x)$，然后我们恰好从$x = 4$开始。正如我们所看到的那样，$f$的梯度接近零。更具体地说，$f'(x) = 1 - \tanh^2(x)$，因此是$f'(4) = 0.0013$。因此，在我们取得进展之前，优化将会停滞很长一段时间。事实证明，这是在引入ReLU激活函数之前训练深度学习模型相当棘手的原因之一。
...
# RMSProp算法
 ...
 相关章节或公式中的关键问题之一，是学习率按预定时间表$\mathcal{O}(t^{-\frac{1}{2}})$显著降低。
...
同之前在 相关章节或公式小节一样，我们使用$1 + \gamma + \gamma^2 + \ldots, = \frac{1}{1-\gamma}$。
...
回想在 相关章节或公式一节中，当我们使用学习率为0.4的Adagrad算法时，变量在算法的后期阶段移动非常缓慢，因为学习率衰减太快。
...
在前面的章节中，我们一直在训练过程中使用随机梯度下降，但没有解释它为什么起作用。为了澄清这一点，我们刚在 相关章节或公式中描述了梯度下降的基本原则。本节继续更详细地说明*随机梯度下降*（stochastic gradient descent）。
...
根据詹森不等式（令 相关章节或公式中$i=t$，$\alpha_i = \eta_t/\sum_{t=1}^T \eta_t$）和$R$的凸性使其满足的$E[R(\mathbf{x}_t)] \geq E[R(\bar{\mathbf{x}})]$，因此，
