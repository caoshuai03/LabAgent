...
回想一下，在计算图 相关章节或公式中的单隐藏层简单网络的参数是
...
以上述简单网络为例：一方面，在前向传播期间计算正则项
 相关章节或公式取决于模型参数$\mathbf{W}^{(1)}$和
$\mathbf{W}^{(2)}$的当前值。
它们是由优化算法根据最近迭代的反向传播给出的。
另一方面，反向传播期间参数 相关章节或公式的梯度计算，
...
简单性以较小维度的形式展现，
我们在 相关章节或公式 讨论线性模型的单项式函数时探讨了这一点。
此外，正如我们在 相关章节或公式 中讨论权重衰减（$L_2$正则化）时看到的那样，
参数的范数也代表了一种有用的简单性度量。
...
回想一下 相关章节或公式中带有1个隐藏层和5个隐藏单元的多层感知机。
当我们将暂退法应用到隐藏层，以$p$的概率将隐藏单元置为零时，
结果可以看作一个只包含原始神经元子集的网络。
比如在 相关章节或公式中，删除了$h_2$和$h_5$，
...
同样，我们使用 相关章节或公式中引入的Fashion-MNIST数据集。
...
考虑一下区分猫和狗的问题：训练数据包括 相关章节或公式中的图像。

图示：区分猫和狗的训练数据

在测试时，我们被要求对 相关章节或公式中的图像进行分类。
...
这是用于二元分类的softmax回归（见 相关章节或公式）的一个特例。
...
1. 使用权重$\beta_i$进行 相关章节或公式 中$\{(\mathbf{x}_1, y_1), \ldots, (\mathbf{x}_n, y_n)\}$的训练。
...
如果源分布$q(y)$是“错误的”，
我们可以根据 相关章节或公式中定义的真实风险中的恒等式进行更正：
...
如果我们想参加Kaggle比赛，首先需要注册一个账户（见 相关章节或公式）。

图示：Kaggle网站

在房价预测比赛页面（如 相关章节或公式 所示）的"Data"选项卡下可以找到数据集。我们可以通过下面的网址提交预测，并查看排名：
...
我们已经在 相关章节或公式中描述了多层感知机（MLP），
现在让我们尝试自己实现一个多层感知机。
为了与之前softmax回归（ 相关章节或公式 ）
获得的结果进行比较，
...
可以直接调用`d2l`包的`train_ch3`函数（参见 相关章节或公式 ），
...
在 相关章节或公式中，
我们介绍了softmax回归（ 相关章节或公式），
然后我们从零开始实现了softmax回归（ 相关章节或公式），
接着使用高级API实现了算法（ 相关章节或公式），
...
我们在 相关章节或公式中描述了仿射变换，
它是一种带有偏置项的线性变换。
首先，回想一下如 相关章节或公式中所示的softmax回归的模型架构。
...
下面，我们以图的方式描述了多层感知机（ 相关章节或公式）。

图示：一个单隐藏层的多层感知机，具有5个隐藏单元
...
曾经sigmoid函数$1/(1 + \exp(-x))$（ 相关章节或公式提到过）很流行，
因为它类似于阈值函数。
...
1. 我们知道$\|\mathbf{w}\|^2 = \mathbf{w}^\top \mathbf{w}$。能找到类似的矩阵方程吗（见 相关章节或公式 中的Frobenius范数）？
