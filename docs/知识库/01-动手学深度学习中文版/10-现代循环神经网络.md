...
图示：在时间步2，选择具有第二高条件概率的词元“C”（而非最高条件概率的词元）

 相关章节或公式中的另一个例子阐述了这个问题。
与 相关章节或公式不同，在时间步$2$中，
我们选择 相关章节或公式中的词元“C”，
它具有*第二*高的条件概率。
由于时间步$3$所基于的时间步$1$和$2$处的输出子序列已从
 相关章节或公式中的“A”和“B”改变为
 相关章节或公式中的“A”和“C”，
因此时间步$3$处的每个词元的条件概率也在 相关章节或公式中改变。
假设我们在时间步$3$选择词元“B”，
于是当前的时间步$4$基于前三个时间步的输出子序列“A”“C”和“B”为条件，
这与 相关章节或公式中的“A”“B”和“C”不同。
因此，在 相关章节或公式中的时间步$4$生成
每个词元的条件概率也不同于 相关章节或公式中的条件概率。
结果， 相关章节或公式中的输出序列
“A”“C”“B”和“&lt;eos&gt;”的条件概率为
$0.5\times0.3 \times0.6\times0.6=0.054$，
这大于 相关章节或公式中的贪心搜索的条件概率。
这个例子说明：贪心搜索获得的输出序列
“A”“B”“C”和“&lt;eos&gt;”
不一定是最佳序列。
...
图示：束搜索过程（束宽：2，输出序列的最大长度：3）。候选输出序列是$A$、$C$、$AB$、$CE$、$ABD$和$CED$
...
1. 在 相关章节或公式的机器翻译问题中应用束搜索。
   束宽是如何影响预测的速度和结果的？
1. 在 相关章节或公式中，我们基于用户提供的前缀，
...
而微观数据只记录较短期的时间动态。
 ...
 相关章节或公式描述了一个具有$L$个隐藏层的深度循环神经网络，
...
像选择超参数这类架构决策也跟 相关章节或公式中的决策非常相似。
...
与 相关章节或公式中定义的`init_rnn_state`函数一样，
...
图示：使用循环神经网络编码器和循环神经网络解码器的序列到序列学习
...
图示：使用循环神经网络编码器-解码器逐词元地预测输出序列。

我们将在 相关章节或公式中介绍不同的序列生成策略。
...
所以 相关章节或公式中乘法项之前的系数用于惩罚较短的预测序列。
