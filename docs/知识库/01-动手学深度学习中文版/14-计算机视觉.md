...
这些边界框被称为*锚框*（anchor box）我们将在 相关章节或公式中设计一个基于锚框的目标检测模型。
...
对于两个边界框，它们的杰卡德系数通常称为*交并比*（intersection over union，IoU），即两个边界框相交面积与相并面积之比，如 相关章节或公式所示。
交并比的取值范围在0和1之间：0表示两个边界框无重合像素，1表示两个边界框完全重合。

图示：交并比是两个边界框相交面积与相并面积之比。
...
下面用一个具体的例子来说明上述算法。
如 相关章节或公式（左）所示，假设矩阵$\mathbf{X}$中的最大值为$x_{23}$，我们将真实边界框$B_3$分配给锚框$A_2$。
然后，我们丢弃矩阵第2行和第3列中的所有元素，在剩余元素（阴影区域）中找到最大的$x_{71}$，然后将真实边界框$B_1$分配给锚框$A_7$。
接下来，如 相关章节或公式（中）所示，丢弃矩阵第7行和第1列中的所有元素，在剩余元素（阴影区域）中找到最大的$x_{54}$，然后将真实边界框$B_4$分配给锚框$A_5$。
最后，如 相关章节或公式（右）所示，丢弃矩阵第5行和第4列中的所有元素，在剩余元素（阴影区域）中找到最大的$x_{92}$，然后将真实边界框$B_2$分配给锚框$A_9$。
...
1. 在 相关章节或公式和 相关章节或公式中修改变量`anchors`，结果如何变化？
...
前面的章节（例如 相关章节或公式— 相关章节或公式）介绍了各种图像分类模型。
...
如 相关章节或公式中所介绍的那样，语义分割是对图像中的每个像素分类。
*全卷积网络*（fully convolutional network，FCN）采用卷积神经网络实现了从图像像素到像素类别的变换 :cite:`Long.Shelhamer.Darrell.2015`。
与我们之前在图像分类或目标检测部分介绍的卷积神经网络不同，全卷积网络将中间层特征图的高和宽变换回输入图像的尺寸：这是通过在 相关章节或公式中引入的*转置卷积*（transposed convolution）实现的。
...
如 相关章节或公式所示，全卷积网络先使用卷积神经网络抽取图像特征，然后通过$1\times 1$卷积层将通道数变换为类别个数，最后在 相关章节或公式中通过转置卷积层将特征图的高和宽变换为输入图像的尺寸。
...
除了坐标刻度不同，双线性插值放大的图像和在 相关章节或公式中打印出的原图看上去没什么两样。
...
本节将介绍迁移学习中的常见技巧:*微调*（fine-tuning）。如 相关章节或公式所示，微调包括以下四个步骤。
...
# 图像增广
 ...
 相关章节或公式提到过大型数据集是成功应用深度神经网络的先决条件。
...
在 相关章节或公式中，我们解释了汇聚层可以降低卷积层对目标位置的敏感性。
...
接下来，我们定义了一个辅助函数，以便于读取图像和应用图像增广。Gluon数据集提供的`transform_first`函数将图像增广应用于每个训练样本的第一个元素（由图像和标签组成），即应用在图像上。有关`DataLoader`的详细介绍，请参阅 相关章节或公式。

接下来，我们[**定义一个辅助函数，以便于读取图像和应用图像增广**]。PyTorch数据集提供的`transform`参数应用图像增广来转化图像。有关`DataLoader`的详细介绍，请参阅 相关章节或公式。
...
我们在CIFAR-10数据集上训练 相关章节或公式中的ResNet-18模型。
回想一下 相关章节或公式中对多GPU训练的介绍。
...
我们在 相关章节或公式中对CIFAR-10数据集做了一个实验。CIFAR-10是计算机视觉领域中的一个重要的数据集。
本节将运用我们在前几节中学到的知识来参加CIFAR-10图像分类问题的Kaggle竞赛，(**比赛的网址是https://www.kaggle.com/c/cifar-10**)。

 相关章节或公式显示了竞赛网站页面上的信息。
为了能提交结果，首先需要注册一个Kaggle账户。

图示：CIFAR-10 图像分类竞赛页面上的信息。竞赛用的数据集可通过点击“Data”选项卡获取。
...
这些图片共涵盖10个类别：飞机、汽车、鸟类、猫、鹿、狗、青蛙、马、船和卡车。
 相关章节或公式的左上角显示了数据集中飞机、汽车和鸟类的一些图像。

### 下载数据集

登录Kaggle后，我们可以点击 相关章节或公式中显示的CIFAR-10图像分类竞赛网页上的“Data”选项卡，然后单击“Download All”按钮下载数据集。
...
在这里，我们基于`HybridBlock`类构建剩余块，这与 相关章节或公式中描述的实现方法略有不同，是为了提高计算效率。
...
在训练开始之前，我们使用 相关章节或公式中描述的Xavier初始化。

我们定义了 相关章节或公式中描述的Resnet-18模型。
...
向Kaggle提交结果的方法与 相关章节或公式中的方法类似，上面的代码将生成一个
...
本次(**比赛网址是https://www.kaggle.com/c/dog-breed-identification**)。
 相关章节或公式显示了鉴定比赛网页上的信息。
需要一个Kaggle账户才能提交结果。

在这场比赛中，我们将识别120类不同品种的狗。
这个数据集实际上是著名的ImageNet的数据集子集。与 相关章节或公式中CIFAR-10数据集中的图像不同，
ImageNet数据集中的图像更高更宽，且尺寸不一。

图示：狗的品种鉴定比赛网站，可以通过单击“数据”选项卡来获得比赛数据集。
...
登录Kaggle后，可以点击 相关章节或公式中显示的竞争网页上的“数据”选项卡，然后点击“全部下载”按钮下载数据集。在`../data`中解压下载的文件后，将在以下路径中找到整个数据集：
...
上述结构与 相关章节或公式的CIFAR-10类似，其中文件夹`train/`和`test/`分别包含训练和测试狗图像，`labels.csv`包含训练图像的标签。
...
我们可以像 相关章节或公式中所做的那样整理数据集，即从原始训练集中拆分验证集，然后将图像移动到按标签分组的子文件夹中。
...
回想一下，这个狗品种数据集是ImageNet数据集的子集，其图像大于 相关章节或公式中CIFAR-10数据集的图像。
...
与 相关章节或公式一样，我们可以读取整理后的含原始图像文件的数据集。
...
因此，我们可以使用 相关章节或公式中讨论的方法在完整ImageNet数据集上选择预训练的模型，然后使用该模型提取图像特征，以便将其输入到定制的小规模输出网络中。
...
与 相关章节或公式中的实验不同，以下内容不重新训练用于特征提取的预训练模型，这节省了梯度下降的时间和内存空间。
...
与 相关章节或公式中的最后一步类似，最终所有标记的数据（包括验证集）都用于训练模型和对测试集进行分类。
...
上面的代码将生成一个`submission.csv`文件，以 相关章节或公式中描述的方式提在Kaggle上提交。
...
在 相关章节或公式中，我们以输入图像的每个像素为中心，生成了多个锚框。
...
回想一下，在 相关章节或公式中，我们将卷积图层的二维数组输出称为特征图。
...
下面，我们介绍一种基于CNN的多尺度目标检测方法，将在 相关章节或公式中实现。

在某种规模上，假设我们有$c$张形状为$h \times w$的特征图。
使用 相关章节或公式中的方法，我们生成了$hw$组锚框，其中每组都有$a$个中心相同的锚框。
例如，在 相关章节或公式实验的第一个尺度上，给定10个（通道数量）$4 \times 4$的特征图，我们生成了16组锚框，每组包含3个中心相同的锚框。
...
根据 相关章节或公式中对感受野的定义，特征图在相同空间位置的$c$个单元在输入图像上的感受野相同：
它们表征了同一感受野内的输入图像信息。
...
在 相关章节或公式，我们将通过一个具体的例子来说明它是如何工作的。
...
1. 根据我们在 相关章节或公式中的讨论，深度神经网络学习图像特征级别抽象层次，随网络深度的增加而升级。在多尺度目标检测中，不同尺度的特征映射是否对应于不同的抽象层次？为什么？
1. 在 相关章节或公式中的实验里的第一个尺度（`fmap_w=4, fmap_h=4`）下，生成可能重叠的均匀分布的锚框。
...
例如， 相关章节或公式中的内容图像为本书作者在西雅图郊区的雷尼尔山国家公园拍摄的风景照，而风格图像则是一幅主题为秋天橡树的油画。
最终输出的合成图像应用了风格图像的油画笔触让整体颜色更加鲜艳，同时保留了内容图像中物体主体的形状。

图示：输入内容图像和风格图像，输出风格迁移后的合成图像

## 方法

 相关章节或公式用简单的例子阐述了基于卷积神经网络的风格迁移方法。
...
以 相关章节或公式为例，这里选取的预训练的神经网络含有3个卷积层，其中第二层输出内容特征，第一层和第三层输出风格特征。

图示：基于卷积神经网络的风格迁移。实线箭头和虚线箭头分别表示前向传播和反向传播
...
图示：R-CNN模型
 ...
 相关章节或公式展示了R-CNN模型。具体来说，R-CNN包括以下四个步骤：
...
图示：Fast R-CNN模型
 ...
 相关章节或公式中描述了Fast R-CNN模型。它的主要计算如下：
...
在Fast R-CNN中提出的兴趣区域汇聚层与 相关章节或公式中介绍的汇聚层有所不同。在汇聚层中，我们通过设置汇聚窗口、填充和步幅的大小来间接控制输出形状。而兴趣区域汇聚层对每个区域的输出形状是可以直接指定的。
...
如 相关章节或公式所示，Mask R-CNN是基于Faster R-CNN修改而来的。
...
* *图像分割*将图像划分为若干组成区域，这类问题的方法通常利用图像中像素之间的相关性。它在训练时不需要有关图像像素的标签信息，在预测时也无法保证分割出的区域具有我们希望得到的语义。以 相关章节或公式中的图像作为输入，图像分割可能会将狗分为两个区域：一个覆盖以黑色为主的嘴和眼睛，另一个覆盖以黄色为主的其余部分身体。
...
在之前的实验，例如 相关章节或公式— 相关章节或公式中，我们通过再缩放图像使其符合模型的输入形状。
...
1. 回想一下 相关章节或公式中对数据增强的描述。图像分类中使用的哪种图像增强方法是难以用于语义分割的？
...
在 相关章节或公式— 相关章节或公式中，我们分别介绍了边界框、锚框、多尺度目标检测和用于目标检测的数据集。
...
回想一下在 相关章节或公式中，通过深度神经网络分层表示图像的多尺度目标检测的设计。
由于接近 相关章节或公式顶部的多尺度特征图较小，但具有较大的感受野，它们适合检测较少但较大的物体。
简而言之，通过多尺度特征块，单发多框检测生成不同大小的锚框，并通过预测边界框的类别和偏移量来检测大小不同的目标，因此这是一个多尺度目标检测模型。

图示：单发多框检测模型主要由一个基础网络块和若干多尺度特征块串联而成。

在下面，我们将介绍 相关章节或公式中不同块的实施细节。
...
回忆 相关章节或公式一节介绍的使用卷积层的通道来输出类别预测的方法，
...
[**完整的单发多框检测模型由五个模块组成**]。每个块生成的特征图既用于生成锚框，又用于预测这些锚框的类别和偏移量。在这五个模块中，第一个是基本网络块，第二个到第四个是高和宽减半块，最后一个模块使用全局最大池将高度和宽度都降到1。从技术上讲，第二到第五个区块都是 相关章节或公式中的多尺度特征块。
...
回想一下，在 相关章节或公式中，一个较接近顶部的多尺度特征块是用于检测较大目标的，因此需要生成更大的锚框。
在上面的前向传播中，在每个多尺度特征块上，我们通过调用的`multibox_prior`函数（见 相关章节或公式）的`sizes`参数传递两个比例值的列表。
...
但是，对于这个回归问题，我们在这里不使用 相关章节或公式中描述的平方损失，而是使用$L_1$范数损失，即预测值和真实值之差的绝对值。
...
到目前为止，我们所见到的卷积神经网络层，例如卷积层（ 相关章节或公式）和汇聚层（ 相关章节或公式），通常会减少下采样输入图像的空间维度（高和宽）。
...
例如， 相关章节或公式解释了如何为$2\times 2$的输入张量计算卷积核为$2\times 2$的转置卷积。

图示：卷积核为 $2\times 2$ 的转置卷积。阴影部分是中间张量的一部分，也是用于计算的输入和卷积核张量元素。
...
与通过卷积核“减少”输入元素的常规卷积（在 相关章节或公式中）相比，转置卷积通过卷积核“广播”输入元素，从而产生大于输入的输出。
我们可以通过 相关章节或公式来构建输入张量`X`和卷积核张量`K`从而[**验证上述实现输出**]。
此实现是基本的二维转置卷积运算。
...
使用 相关章节或公式中相同输入和卷积核张量，将步幅从1更改为2会增加中间张量的高和权重，因此输出张量在 相关章节或公式中。

图示：卷积核为$2\times 2$，步幅为2的转置卷积。阴影部分是中间张量的一部分，也是用于计算的输入和卷积核张量元素。

以下代码可以验证 相关章节或公式中步幅为2的转置卷积的输出。
...
1. 在 相关章节或公式中，卷积输入`X`和转置的卷积输出`Z`具有相同的形状。他们的数值也相同吗？为什么？
